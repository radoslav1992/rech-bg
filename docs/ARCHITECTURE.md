...
€12 / 30k, €29 / 100k, €59 / 250k кредита са текущите месечни планове. Не са изведени от потвърдена публична Cloudflare тарифа за този партньорски модел. Публичната страница за точния модел не беше достъпна при разработката. Валидният договор е предоставената от собственика схема и `notebook` интеграцията.
...
`VideoGeneration` изпраща една заявка към fal Queue API (Средно/Високо) или WaveSpeed Predictions API (Ниско), записва получения request ID и status/result URLs и изчаква чрез Workflow sleep. Стъпките за status/download се повтарят безопасно. Изпращането няма автоматичен retry: `submitted_at` се записва преди външната POST заявка, а наличен `provider_request` се използва отново при възстановяване. При двусмислена загуба на отговора заявката не се изпраща втори път, потребителят получава refund, но доставчикът може вече да е начислил разход на оператора. Това е неизбежен остатъчен риск без гарантиран idempotency key от доставчика. При познат fal request се опитва cancellation; за WaveSpeed локален timeout не гарантира отмяна при доставчика.

Входните медии се предоставят през непрогнозируем токен само за активната видео задача и до 2 часа от създаване. Тези URLs, metadata и provider IDs не се връщат в публичните/user job JSON отговори. Завършване/провал прекратява достъпа. Не се приемат произволни потребителски URLs. HTTPS queue URLs са ограничени до queue.fal.run; WaveSpeed URLs се конструират само към api.wavespeed.ai от валидиран prediction ID, а download до CDN/storage домейни. API ключът никога не се изпраща към файловите сървъри. MP4 се проверява за ftyp, ограничава се до 100 MB и се записва чрез R2 multipart с ограничена памет. Портретите се чистят след завършване или при cleanup. Качени портрети и видео се премахват при изтриване на проект/профил; jobs блокират изтриването, докато работят. Външните доставчици имат собствени политики за съхранение; изтриването в Реч БГ не е твърдение за незабавно изтриване при тях.

За съвместимост преди новата миграция старите аудио routes четат schema-tolerant job записи и избират изрично безопасните полета. Средно/Високо се активират с FAL_KEY, Ниско — с WAVESPEED_API_KEY, всички с VIDEO_GENERATION след миграцията. Новото ниво се пази в video_meta.tier, без промяна на стария CHECK в video_tier; старите заявки запазват routing по старото поле. Cron разпознава правилния Workflow по kind и възстановява dispatch или връща кредити при терминално прекъсване.
