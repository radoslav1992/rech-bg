...
5. Return to the project after background video generation. Edit caption words/times and choose from fourteen visual presets: Karaoke, Marker, Bold, Focus, Classic, Minimal, Neon, Reveal, Bounce (Скок), Outline (Контур), Banner (Лента), Retro (Ретро), Underline (Подчертаване) and Bubble (Балон). Customize accent/text colors, size, capitalization and top/middle/bottom placement. Choose 9:16, 4:5, 1:1 or 16:9 framing with contain or center-crop fit. SRT and VTT export are available after audio generation.
