...
| `heygen` | Medium: Avatar III with a temporary photo avatar. High: Avatar IV directly from the uploaded/library portrait. Both use approved audio, 1080p and preserve the portrait's aspect ratio. Low is unavailable. | `HEYGEN_API_KEY` |
...
Rates stay **900 credits per started second for Medium** and **1,800 for High** (27,000 / 54,000 for 30 seconds), plus the separately approved audio. Photo-avatar preparation is included in the Medium quote; it does not debit the user's wallet separately. Audio, voice samples, product-placement images and caption/export processing are unaffected by this video-only switch. Other features can still require their existing fal/ElevenLabs keys.
