...
3. По подразбиране (`VIDEO_PROVIDER=fal`) добавете **Secret** `FAL_KEY` в Cloudflare Worker → Settings → Variables and Secrets. Този ключ обслужва Средно и Високо качество (Kling Standard/Pro). Не добавяйте `VITE_` префикс. За HeyGen задайте текстова променлива `VIDEO_PROVIDER=heygen` и **Secret** `HEYGEN_API_KEY`; тогава само Високо качество е достъпно (Avatar IV, 1080p, същите 1 800 кредита/сек.). Запазете и деплойнете променливите. [Подробности за превключването](VIDEO-STUDIO.md#choose-the-video-provider). Не е нужна миграция; старите задачи запазват първоначалния си доставчик.
