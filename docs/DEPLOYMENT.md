...
| `STRIPE_PRICE_STARTER` | Variable | Stripe Price ID за €12/месец |
| `STRIPE_PRICE_CREATOR` | Variable | Stripe Price ID за €29/месец |
| `STRIPE_PRICE_STUDIO` | Variable | Stripe Price ID за €59/месец |
...
| --- | --- | ---: | --- |
| `STRIPE_PRICE_STARTER` | Реч БГ — Начало | 1200 | 1 месец |
| `STRIPE_PRICE_CREATOR` | Реч БГ — Създател | 2900 | 1 месец |
| `STRIPE_PRICE_STUDIO` | Реч БГ — Студио | 5900 | 1 месец |

### Преминаване към €12 / €29 / €59 (без съществуващи абонати)

1. В същия Stripe акаунт и **Live** режим добавете нова месечна EUR цена към всеки от трите продукта: €12, €29 и €59, с включен данък (`inclusive`). Не сменяйте само името на старата цена — нужна е нова Price с нов `price_...` ID.
2. В Cloudflare → Worker `rech-bg` → Settings → Variables and Secrets заменете `STRIPE_PRICE_STARTER`, `STRIPE_PRICE_CREATOR` и `STRIPE_PRICE_STUDIO` с новите IDs и запазете/деплойнете. Ключът и webhook secret не се сменят, ако акаунтът и режимът остават същите.
3. Ако Customer Portal позволява смяна на план, обновете предлаганите цени с новите. Проверете дали споделеният Stripe акаунт не използва същата portal конфигурация за друг продукт.
4. Архивирайте старите цени и прекратете останали от тестове отворени Checkout Sessions, ако има такива. Не изтривайте общите продукти или тези на Записки.
5. След deploy отворете нов checkout и проверете сумата, EUR, месечния период и данъка. Активиране на кредити се проверява с платена тестова транзакция/webhook, не само с redirect.

Не е нужна D1 миграция. Квотите остават 30 000 / 100 000 / 250 000; обикновено аудио — 1 кредит/символ, студийно аудио — 3. Новите видео заявки са 300 / 900 / 1 800 кредита/започната секунда. Старите заявки запазват резервираната сума; остаряла цена в отворен таб се отхвърля преди резервиране и потребителят трябва да обнови страницата. Няма автоматична промяна на Stripe абонаменти. Ако впоследствие има абонати при бъдеща смяна на Price IDs, първо трябва да се добави поддръжка на старите IDs във webhook-а.

Докато Cloudflare сочи старите Stripe цени, checkout умишлено отказва несъответстващата сума. Новият idempotency key включва Price ID, така че смяната на настройката не преизползва сесия за различна цена.

Предстоящите продуктови изображения, облачен експорт и новите ограничения за съхранение още не са активирани с тази промяна на цените.
...
3. По подразбиране (`VIDEO_PROVIDER=fal`) добавете **Secret** `FAL_KEY` в Cloudflare Worker → Settings → Variables and Secrets. Този ключ обслужва Средно и Високо качество (Kling Standard/Pro). Не добавяйте `VITE_` префикс. За HeyGen задайте текстова променлива `VIDEO_PROVIDER=heygen` и **Secret** `HEYGEN_API_KEY`: Средно качество използва Kling Avatar V2 Standard през fal (900 кредита/сек.) и изисква `FAL_KEY` и в този режим; Високо — HeyGen Avatar IV (1 800 кредита/сек.). Ниско качество не е налично в този режим. HeyGen ключът трябва да позволява създаване и четене на видеа. Запазете достъпа за създаване, четене и изтриване на аватари до приключване на стари задачи с Avatar III и тяхното почистване. Запазете и деплойнете променливите. [Подробности за превключването](VIDEO-STUDIO.md#choose-the-video-provider). Не е нужна миграция; старите задачи запазват първоначалния си доставчик.
...
5. За Ниско качество създайте акаунт и добавете средства в [WaveSpeedAI](https://wavespeed.ai/), вземете ключ от [API Keys](https://wavespeed.ai/accesskey) и добавете **Secret** `WAVESPEED_API_KEY` в Cloudflare Worker. Стойността е само ключът, без `Bearer `, кавички или `VITE_` префикс. Запазете и деплойнете променливите.
6. Създайте аудио с един глас от 5 до 60 секунди. В секцията **Дайте лице на гласа** изберете запис, качество и качете JPG/PNG портрет до 2 MB с потвърждение за право на използване. Цената се показва преди генерирането. Готовият MP4 е в проекта и се изтегля само след вход от собственика.

Месечните цени са €12 / €29 / €59, с квоти 30 000 / 100 000 / 250 000 кредита. Съществуващите символи стават кредити 1:1 и вече изразходваното остава отчетено. Аудио: 1 символ = 1 кредит. Видео: **Ниско качество** — 300 кредита/сек. (InfiniteTalk Fast, `wavespeed-ai/infinitetalk-fast` през WaveSpeedAI); **Средно качество** — 900 кредита/сек. (`fal-ai/kling-video/ai-avatar/v2/standard`); **Високо качество** — 1 800 кредита/сек. (`fal-ai/kling-video/ai-avatar/v2/pro`). За 30 секунди: 9 000 / 27 000 / 54 000 кредита, допълнително към аудиото. Всяка започната секунда се брои за цяла. Подкасти с два гласа не са вход за един аватар.

Референтни доставни цени: [InfiniteTalk Fast $0.015/сек.](https://wavespeed.ai/models/wavespeed-ai/infinitetalk-fast), [Kling Standard $0.0562/сек.](https://fal.ai/models/fal-ai/kling-video/ai-avatar/v2/standard), [Kling Pro $0.115/сек.](https://fal.ai/models/fal-ai/kling-video/ai-avatar/v2/pro). Следете действителните разходи преди промяна на тарифите в `shared/video.ts`. Сървърът проверява потвърдената цена преди резервиране на кредити.

За преминаване от двата стари към трите нови варианта **не е нужна нова миграция**, ако `0002_avatar_videos.sql` вече е изпълнена. Новите заявки пазят нивото в `video_meta.tier`; старият `video_tier` остава `quality`, за да се запазят CHECK constraint и quota triggers. Старите заявки без `video_meta.tier` продължават през първоначалния си доставчик. Argil вече не се предлага за нови заявки. Нивата се активират независимо според наличния ключ; по подразбиране се избира Средно качество, ако е достъпно.

WaveSpeed използва Bearer authentication, връща prediction ID и се следи чрез `/api/v3/predictions/{id}/result`. Резултатът се записва в частния R2 като MP4. Терминалните `failed`, `cancelled`, `timeout` и `deleted` връщат кредитите. При локален timeout не твърдим, че външната WaveSpeed задача е отменена; няма автоматично повторно платено изпращане.
...
- `SUBMIT_AUTH_401`: проверете `FAL_KEY` за Средно/Високо или `WAVESPEED_API_KEY` за Ниско в production Worker; поставете само стойността на ключа, без `Key ` / `Bearer ` и без кавички. Празни символи в началото/края се премахват автоматично.
