...
3. По подразбиране (`VIDEO_PROVIDER=fal`) добавете **Secret** `FAL_KEY` в Cloudflare Worker → Settings → Variables and Secrets. Този ключ обслужва Средно и Високо качество (Kling Standard/Pro). Не добавяйте `VITE_` префикс. За HeyGen задайте текстова променлива `VIDEO_PROVIDER=heygen` и **Secret** `HEYGEN_API_KEY`: Средно качество използва Avatar III (900 кредита/сек.), Високо — Avatar IV (1 800 кредита/сек.), и двете с одобреното аудио и 1080p. Ниско качество не е налично в този режим. Ключът трябва да позволява създаване, четене и изтриване на аватари, както и създаване и четене на видеа. Запазете и деплойнете променливите. [Подробности за превключването](VIDEO-STUDIO.md#choose-the-video-provider). Не е нужна миграция; старите задачи запазват първоначалния си доставчик.
